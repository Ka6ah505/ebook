...
local optionText = {
	text = "Так как у Грибочка никогда не было друзей, эта нечаянная встреча стала для него самой счастливой и радостной!",
	x = 680,
	y = 1400,
	width = 1300,
...
local function onPageSwap( event )
    ...
    elseif event.phase == "ended" or event.phase == "cancelled" then
    	...
    	if distance > swipeThreshNext then
	    	storyboard.removeScene( "scene9" )
			storyboard.gotoScene( "end", "fade", 400  )
		elseif distance < swipeThreshPrev and distance < 0 then
			storyboard.removeScene( "scene9" )
			storyboard.gotoScene( "scene8", "fade", 400  )
	    end
...
function scene:createScene( event )
	...
	background = display.newImageRect( "scene9.png", display.contentWidth, display.contentHeight)
